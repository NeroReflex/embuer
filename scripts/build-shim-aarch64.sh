...
DOCKERFILE="$ROOT_DIR/Dockerfile.aarch64"
TAG="embuer-shim:aarch64"
...
# force aarch64/arm64 platform to ensure shim is built for aarch64
docker build --platform=linux/arm64 -f "$DOCKERFILE" -t "$TAG" "$ROOT_DIR"
...
# For aarch64 builds the EFI binaries are typically named BOOTAA64.EFI and mmarm.efi
cp "$OUT_DIR/shim_install/boot/efi/EFI/BOOT/BOOTAA64.EFI" "BOOTAA64.EFI" 2>/dev/null || true
cp "$OUT_DIR/shim_install/boot/efi/EFI/BOOT/mmarm.efi" "mmarm.efi" 2>/dev/null || true
