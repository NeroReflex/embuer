...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 4 {
        eprintln!(
            "Usage: {} <public_key.pem> <file_to_verify> <signature_file>",
            args[0]
        );
        eprintln!(
            "Example: {} pub.pem update.btrfs.xz update.signature",
            args[0]
        );
        std::process::exit(1);
    }
    ...
    println!(
        "   ✓ Public key loaded (key size: {} bits)\n",
        pubkey.n().bits()
    );
...
fn verify_signature(
    ...
    println!(
        "   First 50 bytes of decrypted (hex): {}",
        hex::encode(&padded_bytes[..padded_bytes.len().min(50)])
    );
    ...
    if sep_idx >= padded_bytes.len() || padded_bytes[sep_idx] != 0x00 {
        ...
        println!(
            "   Byte at index {}: {:02x}",
            sep_idx,
            if sep_idx < padded_bytes.len() {
                padded_bytes[sep_idx]
            } else {
                0
            }
        );
    ...
    println!(
        "   Expected DigestInfo (hex): {}",
        hex::encode(sha512_digest_info)
    );
    println!(
        "   Found DigestInfo (hex):    {}",
        hex::encode(found_digest_info)
    );
